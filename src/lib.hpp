...
namespace shm_pm {
    ...
    class SharedMapView {
        private:
            ...
            const PaddedSlot<T, ALIGN>* slot_table = nullptr; // 指向 Slot 数组
        ...
        public:
            ...
            __attribute__((always_inline)) const T* get(uint64_t key) const {
                ...
                uint64_t seed = control_table[h >> local_bucket_shift];
                uint64_t slot_idx = (h ^ seed) & local_slot_mask;
                const auto& slot = slot_table[slot_idx];
                if (__builtin_expect(slot.key == key, 1)) {
                    return &slot.value;
                }
                return nullptr;
            }
    ...
    class ShmMapStorage {
        private:
            ...
            BuildResult build_perfect_hash_tables(const std::vector<std::pair<uint64_t, T>>& data) {
                ...
                BuildResult res;
                double slot_factor = 1.15; // 稍微增加因子以降低构建难度
                double bucket_factor = 0.6;
                    ...
                    for (const auto& bucket : buckets) {
                        ...
                        for (int attempt = 0; attempt < 2000000; attempt++) {
                            ...
                            if (!collision) {
                                ...
                                for (size_t i = 0; i < bucket.data_indices.size(); i++) {
            ...
            bool try_create_new(const std::vector<std::pair<uint64_t, T>>& data) {
                ...
                // [修改] 计算内存布局: Header + Control + Slots(Key+Value)
...

                {
                    ...
                    ss << "开始创建共享内存 (Perfect Hash Map - AoS Optimized):\n"
                       ...
                       << "  sizeof(Slot)    = " << sizeof(SlotType) << " 字节 (含 Key+Value)\n"
                       << "  alignof(T)      = " << alignof(T) << "\n"
...

                {
                    ...
                    ss << "内存布局计算详情:\n"
                       ...
                       << "  [3] Slot 表       = " << slots_sz << " 字节 (合并了 Key 和 Value)\n"
                       << "  --------------------------------\n"
                       << "  实际数据总大小    = " << total_sz << " 字节\n"
                ...
                // 初始化所有 Slot 为空 Key
                for (size_t i = 0; i < hdr->slot_count; ++i) {
                ...
                for (size_t i = 0; i < build_res.value_indices.size(); i++) {
